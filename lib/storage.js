...
// Copy selected fields by category/field index. Keeping the original category
// structure avoids surprising changes to a target resume's custom sections.
export async function syncProfileFields(sourceId, targetIds, fieldKeys) {
  const profiles = await getProfiles();
  const source = profiles.find(p => p.id === sourceId);
  if (!source || !Array.isArray(targetIds) || !fieldKeys?.length) return 0;
  const selected = new Set(fieldKeys);
  let changed = 0;
  for (const target of profiles) {
    if (!targetIds.includes(target.id) || target.id === sourceId) continue;
    target.categories.forEach((category, categoryIndex) => {
      category.fields.forEach((field, fieldIndex) => {
        const key = `${categoryIndex}:${fieldIndex}`;
        if (!selected.has(key)) return;
        const sourceField = source.categories?.[categoryIndex]?.fields?.[fieldIndex];
        if (!sourceField || field.value === sourceField.value) return;
